...
- **Le PV signé, en PDF** (v1.20.0) : une fois l'élection close, bloc « 📎 Joindre le PV signé » sous le résumé des élus → `election.pv = { nom, fichier, taille }` ; le même bloc sur la désignation sans vote (`cls.delegues.pv`, qui survit à une correction des noms). Même dossier et mêmes règles que les fiches incident (`_pjRef`, `_pvBlocHTML`, `pvAttachUI` / `pvOpenUI` / `pvRemoveUI`), nom automatique `AAAA-MM-JJ PV élection délégués — 5e C.pdf`. `_pjReferences` les compte, donc le nettoyage des orphelins les épargne.
